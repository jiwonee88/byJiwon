...
const rows = [
  ...
  function dialogChange(row){
      console.log(row);
      dialogInfo.name = row.nickname;
      dialogInfo.dialogOpen = true;
  ...
  function StickyHeadTable() {
    ...
    return (
      <Paper className={classes.root}>
          ...
          <Table stickyHeader aria-label="sticky table">
            ...
            <TableBody>
              {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.code} onClick={()=>{dialogChange(row);setOpen(true);}}>
                    {columns.map((column) => {
        ...
        />
        <FullScreenDialog open={open} info={dialogInfo} handleClose={()=>{setOpen(false);}}/>
      </Paper>
    );
  }
